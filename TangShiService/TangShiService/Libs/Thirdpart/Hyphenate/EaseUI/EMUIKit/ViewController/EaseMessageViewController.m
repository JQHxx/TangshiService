                     ...
                     timestamp:(long long)timestamp
{
    ...
    [self.conversation loadMessagesStartFromId:messageId count:(int)count searchDirection:EMMessageSearchDirectionUp completion:^(NSArray *aMessages, EMError *aError) {
        if (!aError && [aMessages count]) {
            ...
            [self _loadMessagesFromMyServerBefore:messageId count:count append:isAppend timestamp:timestamp];
        }
    }];
}
...
- (void)_loadMessagesFromMyServerBefore:(NSString*)messageId count:(NSInteger)count append:(BOOL)isAppend timestamp:(long long)timestamp{
    ...
    NSString *body=[NSString stringWithFormat:@"timestamp=%lld&group_id=%@&page_size=%ld&sort=desc",timestamp,self.userModel.im_groupid,(long)count];
    [[TCHttpRequest sharedTCHttpRequest] postMethodWithURL:kChatMessageList body:body success:^(id json) {
        ...
        if (kIsArray(list)&&list.count>0) {
            ...
            for (NSDictionary *dict in list) {
                ...
                EMMessage *message = [[EMMessage alloc] initWithConversationID:self.userModel.im_groupid from:chat.from to:chat.to body:messageBody ext:nil];
...
-(void)moreViewQuestionnaireAction:(EaseChatBarMoreView *)moreView{
    ...
    if ([self.delegate respondsToSelector:@selector(messageViewControllerMoreActionAddQuestionnaire:)]) {
        [self.delegate messageViewControllerMoreActionAddQuestionnaire:self];
    }
}
...
#pragma mark 发送检查表消息
-(void)sendQuestionnarieMessage:(NSDictionary *)questionnarieDict{
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
        progress = [_dataSource messageViewController:self progressDelegateForMessageBodyType:EMMessageBodyTypeVideo];
    }
    ...
    NSDictionary *dicExt=[[NSDictionary alloc] initWithObjectsAndKeys:questionnarieDict,@"msg_info",@"question",@"msg_type",nil];
    EMMessage *message = [EaseSDKHelper sendTextMessage:[questionnarieDict valueForKey:@"name"]
                                                     to:self.conversation.conversationId
                                            messageType:[self _messageTypeFromConversationType]
                                             messageExt:dicExt];
    [self _sendMessage:message];
