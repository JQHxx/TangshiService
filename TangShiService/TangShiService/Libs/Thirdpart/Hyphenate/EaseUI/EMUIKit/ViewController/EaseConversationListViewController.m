...
- (void)tableViewDidTriggerHeaderRefresh
{
    ...
    MyLog(@"获取会话%ld条",(long)conversations.count);
    ...
    for (EMConversation *converstion in sorted) {
        ...
        if (self.dataSource && [self.dataSource respondsToSelector:@selector(conversationListViewController:modelForConversation:)]) {
            model = [self.dataSource conversationListViewController:self modelForConversation:converstion];
        }
        else{
        ...
        if (model) {
            MyLog(@"会话标示ID：%@,未读消息数:%d,拓展属性:%@",model.conversation.conversationId,model.conversation.unreadMessagesCount,model.conversation.ext);
